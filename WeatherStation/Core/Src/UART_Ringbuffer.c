 ...
 * @brief       : This file implements a UART ring buffer for efficient UART communication.
 *  Created on  : 10-Jul-2019
 *  Author      : Controllerstech
 *
 * It includes initialization of ring buffers for transmission and reception,
 * functions to store and retrieve characters, and utilities to handle UART data.
 * The module operates with two UART handlers, one for the device (huart1) and
 * one for the PC (huart2), and utilizes ring buffers to manage incoming and outgoing data.
 * Functions are provided to look for specific strings in the buffer, copy data between buffers,
 * flush UART data, peek at the next byte, read and write UART data, check data availability,
 * and send strings or numbers over UART. An ISR (Interrupt Service Routine) is implemented
 * for efficient handling of UART interrupts.
 *
 */
...
#define device_uart &huart1					/**< Macro defining the UART interface for device communication.*/
#define pc_uart &huart2						/**< Macro defining the UART interface for PC communication.*/
